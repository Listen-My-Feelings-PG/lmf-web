    ...
    try {
      await psql`
        INSERT INTO public.calibracion (
          ...
          ${data.userScore},
          ${data.configEpochs},
          ${data.probScore0 ?? null},
          ${data.probScore1 ?? null},
          ${data.probScore2 ?? null},
          ${data.probScore3 ?? null},
          ${data.loss ?? null},
          ${data.accuracy ?? null},
  ...
  static async createBatch(entries: Omit<Calibration, 'id'>[]): Promise<void> {
    ...
    try {
      const rows = entries.map(e => ({
        ...
        cl_ts_calif_global: e.globalScore ?? null,
        cl_user_score: e.userScore,
        cl_ts_config_epocas: e.configEpochs,
        cl_ts_prob_calif_0: e.probScore0 ?? null,
        cl_ts_prob_calif_1: e.probScore1 ?? null,
        cl_ts_prob_calif_2: e.probScore2 ?? null,
        cl_ts_prob_calif_3: e.probScore3 ?? null,
        cl_loss: e.loss ?? null,
      ...
      await psql`
        ...
        'cl_ts_config_epocas', 'cl_ts_prob_calif_0', 'cl_ts_prob_calif_1',
        'cl_ts_prob_calif_2', 'cl_ts_prob_calif_3', 'cl_loss', 'cl_accuracy',
